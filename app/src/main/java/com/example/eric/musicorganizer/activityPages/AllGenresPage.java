...
public class AllGenresPage extends AppCompatActivity implements SearchView.OnQueryTextListener, ServiceConnection {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_all_genres_page);
        initActivityDisplay();
    }
    ...
    private void initActivityDisplay() {
        ListView genreListView = findViewById(R.id.genres_page_list);
        ...
        genreAdapter = new GenreAdapter(this, genresList);    // get view adapter
        genreListView.setAdapter(genreAdapter);                       // it should present the list of songs on the device
        genreListView.setOnItemClickListener((parent, view, position, id) -> onItemClick(position));
    }

    private void onItemClick(int position) {
        Intent intent = new Intent(getBaseContext(), ArtistsByGenrePage.class);
        intent.putExtra(getResources().getString(R.string.GenreNameKey), genreAdapter.getItem(position).getName());
        startActivity(intent);
    }

    public void getSongList() {
        MusicManager.getInstance().sortGenresList();
        genresList = MusicManager.getInstance().getGenresList();
    }
